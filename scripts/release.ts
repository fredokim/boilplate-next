...
const GATES: readonly Gate[] = [
  ...
  { name: "generators", proves: "each generator produces what FEATURE_CONTRACT.md requires", ...npm("check:generators") },
  ...
  /**
   * No image gate. This repository deploys on Render's Node runtime rather than
   * from a Dockerfile — `npm ci && npm run build`, then `npm run start` — so
   * there is no image to build and forcing one would be a gate that proves
   * nothing about how this actually ships.
   *
   * The equivalent question, whether `npm run start` serves the application and
   * whether the backend's address stayed out of the client bundle, is the
   * `runtime` job in CI. It needs a stub upstream and a real port, which is a
   * job rather than a gate.
   */
];
