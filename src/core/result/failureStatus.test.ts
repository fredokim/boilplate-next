...
import { TypedAppError } from "./failure";
...
describe("failureKeyOf", () => {
  ...
  it("separates sign-in-again from you-may-not", () => {
    expect(failureKeyOf({ origin: "backend", kind: "unauthorized", message: "" })).toBe("unauthorized");
    expect(failureKeyOf({ origin: "backend", kind: "forbidden", message: "" })).toBe("forbidden");
    expect(failureKeyOf({ origin: "auth", kind: "unknown", message: "", status: 403 })).toBe("forbidden");
  });

  it("reads a contract mismatch as the server surprising us, not the reader erring", () => {
    expect(failureKeyOf({ origin: "frontend-contract", kind: "unknown", message: "" })).toBe("contract");
    expect(failureKeyOf({ origin: "backend", kind: "validation", message: "" })).toBe("contract");
...
describe("describeFailure", () => {
  it("accepts whatever was thrown", () => {
    expect(describeFailure(new TypedAppError({ origin: "backend", kind: "server", message: "boom" })).title).toBe(
      "Server error",
    );
