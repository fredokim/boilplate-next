...
import type { EdgeRuntimeState, EdgeRuntimeStatus, NodeRuntimeState, NodeRuntimeStatus } from "../realtime/types";
...
const runtimeEdgeClass: Partial<Record<EdgeRuntimeStatus, string | undefined>> = {
  disconnected: styles.edgeRuntimeDisconnected,
  degraded: styles.edgeRuntimeDegraded,
};
...
>(
  {
    ...
    runtimeFilter = "all",
    validationErrors = [],
  }: GraphCanvasProps<TNodeType, TNodeMetadata, TEdgeMetadata>,
  ...
  const invalidNodeIds = useMemo(
    () => new Set(validationErrors.filter((error) => error.targetType === "node").map((error) => error.targetId)),
    [validationErrors],
  );
  const invalidEdgeIds = useMemo(
    () => new Set(validationErrors.filter((error) => error.targetType === "edge").map((error) => error.targetId)),
    [validationErrors],
  );
    ...
    () =>
      graph.edges.map((edge) => {
        ...
        return {
          ...
          className: [
            visualState.dimmed ? styles.edgeDimmed : "",
            visualState.hovered ? styles.edgeHovered : "",
            invalidEdgeIds.has(edge.id) ? styles.edgeError : "",
            runtimeState ? (runtimeEdgeClass[runtimeState.status] ?? "") : "",
          ].join(" "),
          markerEnd: { type: MarkerType.ArrowClosed, color },
          style: {
            stroke: color,
            strokeWidth: visualState.routeActive || visualState.hovered ? 3 : 2,
            strokeDasharray: runtimeState?.status === "disconnected" ? "6 4" : undefined,
          },
        };
      }),
  ...
  useImperativeHandle(ref, () => ({
    ...
    focusNode: (nodeId) =>
      void instanceRef.current?.fitView({ duration: 350, maxZoom: 1.35, nodes: [{ id: nodeId }], padding: 1.5 }),
    focusRoute: (nodeIds) =>
  ...
  return (
    <div aria-label="Network topology graph" className={styles.canvas}>
      <ReactFlow
        edges={edges}
        elementsSelectable
        fitView
        ...
        onMoveEnd={(_event, viewport) => {
          ...
          onDebugChange?.({ renderCount: renderCount.current, zoom: viewport.zoom });
        }}
